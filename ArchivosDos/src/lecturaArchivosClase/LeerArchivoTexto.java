...
public class LeerArchivoTexto
{
   ...
   public void abrirArchivo()
   {
      try
      {
         entrada = new Scanner( new File( "clientes3.txt" ) ); //abrir el archivo, File es propio de java. - Genera un archivo de tipo Scanner
      } // fin de try
   ...
   public void leerRegistros()
   {
      ...
      System.out.printf( "%-9s%-15s%-18s%10s\n", "Cuenta", //poner posiciones a la cadena 
         "Primer nombre", "Apellido paterno", "Saldo" );

      try // lee registros del archivo, usando el objeto Scanner
      {
         while ( entrada.hasNext() ) //Sigue leyendo las lineas hasta que no encuentre mas
         {
            registro.establecerCuenta( entrada.nextInt() ); // lee el numero de cuenta
            registro.establecerPrimerNombre( entrada.next() ); // lee el primer nombre - El next lee una cadena hasta que encuentra un espacio
